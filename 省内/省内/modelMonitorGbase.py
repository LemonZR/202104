...
def initSql(dictCofnig, args_period):
    ...
    if 'N' in column:
        ...
        sumSql = "select '%s',concat(%s),count(1),'%s' from %s where %s=%s" % (
            dictCofnig['table_name'], summer, args_period, dictCofnig['table_name'], dictCofnig['period_col'], args_period)
    # print(sumSql)
    return sumSql
...
def args_periods(args_period):
    ...
    if len(args_period) == 8:
        ...
        num = int(args_period) - 202102
        for i in range(num):
            periods.append(add_month('202102', i))
        return periods
...
def mainFunc(args):
    ...
    if len(args_period) == 8:
        insertTable = 'pub.tdet_hb_modeldatamonitor_d'
        errorTable = 'pub.tdet_hb_modeldatamonitor_error_d'
        statisDate = 'statis_date'
    else:
        insertTable = 'pub.tdet_hb_modeldatamonitor_m'
        errorTable = 'pub.tdet_hb_modeldatamonitor_error_m'
    ...
    for dd in unionSqls:
        ...
        try:
            ...
            insertError = "insert into %s values ('SqlError','%s','%s','%s')" % (errorTable, str(talbes), args_period, args_period)
            print(insertError)
            ...
            try:
                if 'T' in row:
                    ...
                    insertSql = "insert into %s values('%s','%s','%s','%s','%s','%s')" % (
                        insertTable, dictResult['table_name'], row[3], str(dictResult['total']), 'total', str(dictResult['total']), row[3])
                    print(insertSql)
                ...
                insertError = "insert into %s values ('dataError','%s','%s','%s')" % (errorTable, str(row[0]), row[3], args_period)
                print(insertError)
                cursor_gbase.execute(insertError)
