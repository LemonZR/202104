...
def get_files_layer(dir_name='D:\\tmp\\'):
...
def has_pattern(layer_dict={}, layer_num=0, pattern=r'union\s*all'):
    pattern = pattern
    result_dict = {}
...
def get_target_table(layer_dict={}, layer_num=0):
    ...
    for blk_id, sql_layer_info in layer_dict.items():
        ...
        if insert_blk:
            target_table_name = re.findall(target_pattern, insert_blk[0], re.I)
            if target_table_name:
                result_dict.setdefault(target_table_name[0], sql_layer_info)
    return result_dict


def run(dir_name):
    file_layers_info = get_files_layer(dir_name)
    union_all_pattern = r'union\s*all'
    xlsx_data = [('脚本名', '目标表', 'sql语句块第0层')]
    for file_name, layer_info in file_layers_info.items():
        union_all_layer = has_pattern(layer_info, pattern=union_all_pattern)
        result = get_target_table(union_all_layer)
        for target_table, sql_layer_info in result.items():
            xlsx_data.append((file_name, target_table.lower(), sql_layer_info[0]))
